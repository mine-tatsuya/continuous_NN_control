# -*- coding: utf-8 -*-
"""
NNPFC training script
- Inputs : ud, udd, yad, yadd
- Target : ya - y  (PFC補正量)
Recipe:
...
# ===== ネットワーク構成（変更しやすいように） =====
HIDDEN_LAYERS = [64, 64]  # 隠れ層の構成 [64, 64] or [30, 20, 10] など自由に変更可能

# ===== 1) 結果保存フォルダ =====
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
ROOT = Path(__file__).resolve().parent.parent
result_dir = ROOT / 'result_NNPFC' / timestamp
os.makedirs(result_dir, exist_ok=True)
print(f'[INFO] Results will be saved to: {result_dir}')

# ===== 2) データ読み込み =====
csv_path = ROOT / 'NN_dataget_continuous' / 'dataget_PFC.csv'
df = pd.read_csv(csv_path)

# 期待ヘッダー: t, ud, udd, yad, yadd, ya, y
expected_cols = ['t', 'ud', 'udd', 'yad', 'yadd', 'ya', 'y']
missing = [c for c in expected_cols if c not in df.columns]
if missing:
    raise ValueError(f'CSVに想定ヘッダーが見つかりません: {missing}\n実ヘッダー: {list(df.columns)}')

# 入力と教師
x_cols = ['ud', 'udd', 'yad', 'yadd']  # PFC入力
y_target = df['ya'].values - df['y'].values  # 目標: ya - y (PFC補正量)

X = df[x_cols].values.astype(np.float32)
y = y_target.astype(np.float32).reshape(-1, 1)

print(f'[INFO] Input features: {x_cols}')
print(f'[INFO] Target: ya - y (PFC correction)')
...
print(f'[INFO] Train: {len(X_tr)}, Val: {len(X_va)}, Test: {len(X_te)}')
...
pd.DataFrame({'target': ['ya_minus_y'],
              'mean':  [float(y_mean)],
...
# ===== 5) モデル定義（L2正則化つき） =====
def build_model(input_dim: int, hidden_layers: list) -> tf.keras.Model:
    """
    ...
    # 最初の隠れ層
    layers.append(Dense(hidden_layers[0], activation='relu', 
                       kernel_regularizer=l2, input_shape=(input_dim,)))
    
    # 残りの隠れ層
    for units in hidden_layers[1:]:
        layers.append(Dense(units, activation='relu', kernel_regularizer=l2))
...
model = build_model(X_tr_s.shape[1], HIDDEN_LAYERS)
print(f'[INFO] Model architecture: input({X_tr_s.shape[1]}) -> {" -> ".join(map(str, HIDDEN_LAYERS))} -> output(1)')
model.summary()
...
# ===== 9) モデル保存（最終：現在の重み） =====
final_model_path = str(result_dir / 'nnpfc_best.keras')
...
for i, layer in enumerate(model.layers):
    ...
    if len(weights) == 2:
        w, b = weights
        np.savetxt(str(result_dir / f'z2pfc_weights_layer_{i}.csv'), w, delimiter=',')
        np.savetxt(str(result_dir / f'z2pfc_biases_layer_{i}.csv'),  b, delimiter=',')
...
print(f'  Target: ya - y')
print(f'  Network: {X_tr_s.shape[1]} -> {" -> ".join(map(str, HIDDEN_LAYERS))} -> 1')
